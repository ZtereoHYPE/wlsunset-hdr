...
static void fill_gamma_table(uint16_t *table, uint32_t ramp_size, 
		struct rgb linear_wp, uint32_t transfer_function, double gamma) {
	uint16_t *r = table;
	uint16_t *g = table + ramp_size;
	uint16_t *b = table + 2 * ramp_size;
	for (uint32_t i = 0; i < ramp_size; ++i) {
		double val = (double)i / (ramp_size - 1);
		struct rgb gamma_val = calc_gamma_value(linear_wp, val, transfer_function);
		r[i] = (uint16_t)(UINT16_MAX * pow(gamma_val.r, 1.0 / gamma));
		g[i] = (uint16_t)(UINT16_MAX * pow(gamma_val.g, 1.0 / gamma));
		b[i] = (uint16_t)(UINT16_MAX * pow(gamma_val.b, 1.0 / gamma));
	}
}

static void output_set_whitepoint(struct output *output, struct rgb linear_wp, double gamma) {
	if (!output->enabled || output->gamma_control == NULL || output->table_fd == -1) {
		return;
	}
	fill_gamma_table(output->table, output->ramp_size, linear_wp, 
			output->transfer_function, gamma);
...
static void set_temperature(struct wl_list *outputs, int temp, double gamma) {
	...
	struct xyz wp = calc_whitepoint_xyz(temp);
	...
	wl_list_for_each(output, outputs, link) {
		...
		struct rgb linear_wp = calc_whitepoint_linear(wp, output->primaries);
		output_set_whitepoint(output, linear_wp, gamma);
	}
}
